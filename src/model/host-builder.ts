...
class HostBuilder {
  ...
  private static async runWindows(actionFolder: string, silent: boolean): Promise<number> {
    const buildScript = `${actionFolder}/platforms/windows/local/build.ps1`;

    // const command = `Powershell.exe -File "${buildScript}" -ExecutionPolicy Bypass -NoProfile -NonInteractive`;
